...
class CoreMemoryRecallTool(FunctionTool):
    ...
    async def run(
        ...
        self.logger.debug(f"{self.name} - LLM 调用: limit={limit}, query='{query}', time_range='{time_range}'")
        ...
        try:
            ...
            if time_range and time_range in ["today_morning", "today_afternoon", "yesterday", "past_7_days"]:
                ...
                if sql_manager and hasattr(sql_manager, "recall_by_time"):
                    ...
                    time_memories = await sql_manager.recall_by_time(time_range, memory_scope)
                    
                    if time_memories:
                        self.logger.info(f"{self.name}: 时间检索命中 {len(time_memories)} 条记录。")
                        display_memories = [
                            MemoryItem(
                            ...
                            ) for mem in time_memories
                        ]
                        return f"【基于时间维度的记忆检索结果】\n" + MemoryFormatter.format_session_memories(display_memories)
                    else:
                        return f"未找到 '{time_range}' 时间段内的相关记忆。"
            # --- 结束新增 ---

            # 原有的常规向量与BM25混合检索（作为时间查询未触发时的默认流转）
