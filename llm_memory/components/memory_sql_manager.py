...
class MemorySqlManager:
    ...
    def _init_db(self) -> None:
        with self._connect() as conn:
            ...
            if "is_archived" not in memory_column_names:
                cur.execute(
                    "ALTER TABLE memory_records ADD COLUMN is_archived INTEGER NOT NULL DEFAULT 0"
                )
            # --- 结束新增 ---
    ...
    async def consolidate_memories(self) -> None:
        ...
        with self._connect() as conn:
            # 找到衰减到 0 且尚未归档的记忆
            rows = conn.execute(
                "SELECT id FROM memory_records WHERE is_active = 0 AND strength <= 0 AND is_archived = 0"
            ).fetchall()
            deleted_ids = [str(row["id"]) for row in rows]
            
            # --- 第三批次修改：不再物理删除，改为逻辑归档 (is_archived = 1) ---
            conn.execute("UPDATE memory_records SET is_archived = 1 WHERE is_active = 0 AND strength <= 0 AND is_archived = 0")
            # 保留 tags 关联关系，以备特殊时间回溯时使用
            conn.commit()
